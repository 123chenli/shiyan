...
//ON_EN_CHANGE(IDC_EDIT_NumOfImg, &CbinocularMeasureDSMFCDlg::OnChangeEditNumofimg)
END_MESSAGE_MAP()
...
BOOL CbinocularMeasureDSMFCDlg::OnInitDialog()
{
	...
	m_COMBO_MethodList.AddString(_T("Canny边缘检测"));
	m_COMBO_MethodList.AddString(_T("灰度处理"));
	m_COMBO_MethodList.AddString(_T("高斯模糊"));
	...
	// 初始化图像显示控件的图像
	m_lfImage = Mat::zeros(m_nImageHeight, m_nImageWidth, CV_8UC3);
	m_riImage = Mat::zeros(m_nImageHeight, m_nImageWidth, CV_8UC3);
	m_disparity = Mat::zeros(m_nImageHeight, m_nImageWidth, CV_8UC3);
...
void CbinocularMeasureDSMFCDlg::OnBnClickedBtnExecute()
{
	...
	if (parm_selected)
	{
		...
		fs.open("parms//calibParms.xml", FileStorage::READ);
...
BOOL CbinocularMeasureDSMFCDlg::PreTranslateMessage(MSG* pMsg)
{
	...
	if(pMsg-> message==WM_LBUTTONDOWN)     
	{
		...
		if (rc_disparity.PtInRect(ptCursor))
		{
			...
			float  distance = - pointClouds.at<cv::Point3f>(yy,xx).z;
			if ( distance >=0 )
			{
				CString str;
				str.Format(_T("The distance at ( %d , %d ) is %.3f cm "), int(xx), int(yy), distance);
...
void CbinocularMeasureDSMFCDlg::OnBnClickedBtnSelectparmfolder()
{
	...
	if (xmlFiles.size() == 0)
	{
		AfxMessageBox(_T("没有选中有效的摄像头定标参数文件!"));
		return;
	}
...
void CbinocularMeasureDSMFCDlg::OnBnClickedBtnClosecam()
{
	...
	Mat left_tmp = cv::Mat::zeros(m_nImageHeight, m_nImageWidth, CV_8UC3);
	Mat right_tmp = cv::Mat::zeros(m_nImageHeight, m_nImageWidth, CV_8UC3);
	m_showImg_left.ShowImage(left_tmp, 1);
	m_showImg_right.ShowImage(right_tmp, 1);
...
void CbinocularMeasureDSMFCDlg::OnBnClickedBtnSnapshot()
{
	...
	char leftimg_file[200];
	char rightimg_file[200];

	const char *left_directory0 = NULL;
	const char *right_directory0 = NULL;
	char left_directory[200] = "";
	char right_directory[200] = "";
	...
	sprintf(leftimg_file, "%s//%s%d.%s", left_directory, leftimg_filename, writeImgindex, extension);
	sprintf(rightimg_file, "%s//%s%d.%s", right_directory, rightimg_filename, writeImgindex, extension);

	imwrite(leftimg_file, m_lfImage);
	imwrite(rightimg_file, m_riImage);

	Mat left_tmp = Mat::zeros(m_nImageHeight, m_nImageWidth, CV_8UC3);
	Mat right_tmp = Mat::zeros(m_nImageHeight, m_nImageWidth, CV_8UC3);
	m_showImg_left.ShowImage(left_tmp, 1);
	m_showImg_right.ShowImage(right_tmp, 1);

	CString str;
	str.Format(_T("成功保存第 %d 对图片 %s 和 %s"), writeImgindex, CString(leftimg_file), CString(rightimg_file));
	m_ctrl_edit_show.SetSel(-1, -1);				//自动滚屏
	m_ctrl_edit_show.ReplaceSel(str + "\r\n");		//自动换行

	m_edit_NumOfImg = writeImgindex;
	writeImgindex++;
...
void CbinocularMeasureDSMFCDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	switch (nIDEvent)
	{
		case 1:
		{
			...
			{
				DoFrameProc(imageLeft, m_lfImage);
				DoFrameProc(imageRight, m_riImage);
		...
		case 2:
		{
			...
			Mat frameLeft0 = Mat(lfCam.QueryFrame());
			Mat frameRight0 = Mat(riCam.QueryFrame());
			Mat frameLeft = cv::Mat(m_nImageHeight, m_nImageWidth, CV_8UC3);
			Mat frameRight = cv::Mat(m_nImageHeight, m_nImageWidth, CV_8UC3);

			if (frameLeft0.empty() || frameRight0.empty()) return;
			if (m_nImageWidth == 0 || m_nImageHeight == 0) return;

			if (m_ProcMethod != SHOW_ORIGINAL_FRAME && m_ProcMethod != SHOW_GRAY_FRAME)
			{
				DoFrameProc(frameLeft0, frameLeft);
				DoFrameProc(frameRight0, frameRight);
			}
			else
			{
				frameLeft = frameLeft0;
				frameRight = frameRight0;
			}
			...
			str2.Format(_T("计算完成，鼠标左击视差图输出距离"));
			str3.Format(_T("点击 视差计算 进行计算下一对"));
		...
		case 3:
		{
			...
			if (picIndex <= m_edit_NumOfImg)
			{
				...
				//获取一帧
				Mat frameLeft0, frameRight0;
				Mat frameLeft = cv::Mat(m_nImageHeight, m_nImageWidth, CV_8UC3);
				Mat frameRight = cv::Mat(m_nImageHeight, m_nImageWidth, CV_8UC3);

				if (img_Loaded)
				{
					frameLeft0 = m_lf_imgs[k];
					frameRight0= m_ri_imgs[k];
				}
				else
				{
					char leftimg_file[200];
					...
					sprintf(leftimg_file, "imgs//%s//%s%d.%s", leftimg_directory, leftimg_filename, k, extension);
					sprintf(rightimg_file, "imgs//%s//%s%d.%s", rightimg_directory, rightimg_filename, k, extension);

					//获取一帧
					frameLeft0 = imread(leftimg_file, CV_LOAD_IMAGE_COLOR);
					frameRight0 = imread(rightimg_file, CV_LOAD_IMAGE_COLOR);
				}
				...
				str3.Format(_T("计算完成，鼠标左击视差图输出距离"));
				str4.Format(_T("点击 视差计算 进行计算下一对"));
				GetDlgItem(IDC_EDIT_Show)->SetWindowText(str1 + "\r\n" + str2 + "\r\n" + str3 + "\r\n" + str4 + "\r\n");
...
void CbinocularMeasureDSMFCDlg::OnBnClickedBtnCalib()
{
	...
	for (int i = 1; i <= m_edit_NumOfImg; i++)
	{
		...
		{
			char lf_img_file[200], ri_img_file[200];
			sprintf(lf_img_file, "calib//%s//%s%d.%s", leftimg_directory, leftimg_filename, i, extension);
			sprintf(ri_img_file, "calib//%s//%s%d.%s", rightimg_directory, rightimg_filename, i, extension);
		...
		{
			AfxMessageBox(_T("读入图片错误,请确保图片格式正确！"));
			return ;
		}
	...
	CString str1, str2, str3, str4;
	str1.Format(_T("标定完成！"));
	str2.Format(_T("左摄像头标定参数已写入 %s"), CString(lf_out_file));
	str3.Format(_T("右摄像头标定参数已写入 %s"), CString(ri_out_file));
	str4.Format(_T("立体标定参数已写入 %s"), CString(stereo_out_file));
	m_ctrl_edit_show.SetSel(-1, -1);																//自动滚屏
	m_ctrl_edit_show.ReplaceSel(str1 + "\r\n" + str2 + "\r\n" + str3 + "\r\n" + str4 + "\r\n");		//自动换行
...
void CbinocularMeasureDSMFCDlg::OnBnClickedBtnReadimg()
{
	...
	for (int i = 1; i <= m_edit_NumOfImg; i++)
	{
		...
		{
			AfxMessageBox(_T("加载图像错误，请确保文件格式正确!"));
			return;
		}
...
void CbinocularMeasureDSMFCDlg::doMatch(Mat frameLeft, Mat frameRight, Mat &disp, Mat &imageLeft, Mat &imageRight)
{
	...
	else if (m_nID_RADIO == IDC_RADIO_SGBM)
	{
		...
		SGBM.P1 = 8 * frameLeft.channels() * SGBM.SADWindowSize * SGBM.SADWindowSize;
		SGBM.P2 = 32 *  frameLeft.channels() * SGBM.SADWindowSize * SGBM.SADWindowSize;
...
//void CbinocularMeasureDSMFCDlg::OnChangeEditNumofimg()
//{
